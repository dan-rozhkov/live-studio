    ...
    <Section title="Components" defaultOpen={false}>
      <div class={styles.componentList}>
        {entries.map(([name, props]) => (
          <div class={styles.component} key={name}>
            <div class={styles.componentName}>{name}</div>
            <div class={styles.componentProps}>
              {Object.entries(props).map(([k, v]) => {
                const { value, unresolved } = resolveRef(doc, v);
                return (
                  <Fragment key={k}>
                    <div class={styles.componentKey}>{k}</div>
                    <div class={styles.componentValue}>
                      {unresolved ? <span class={styles.broken}>↯ {unresolved}</span> : value}
                    </div>
                  </Fragment>
                );
              })}
